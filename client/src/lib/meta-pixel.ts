...
function sendCAPIEvent(eventName: string, eventData: Record<string, unknown>, eventId?: string) {
  ...
  console.log(`[CAPI] Sending ${eventName} (event may be WRONG for this page context)${testEventCode ? ` [TEST: ${testEventCode}]` : ''} — payload:`, JSON.parse(JSON.stringify(payload)));
  fetch(endpoint, {
    method: 'POST',
